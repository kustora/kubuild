...
/**
 * The layer tree. Always renders inline and fills its host container — the left
 * sidebar's "Layers" tab and the mobile drawer own its visibility and chrome.
 */
...
export const LayersPanel: React.FC<LayersPanelProps> = ({ registry, className }) => {
  const { document, selectedNodeId, hoveredNodeId, selectNode, hoverNode, moveComponent } =
    useEditorStore();
  ...
  return (
    ...
    >
      <div className="flex-1 overflow-y-auto p-1.5 text-xs">{renderRow(document.document, 0)}</div>
    </div>
  );
};
