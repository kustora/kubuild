...
export function normalizeRating(value: number, max: number): { value: number; max: number } {
  const safeMax = Math.min(
    Math.max(Math.round(Number.isFinite(max) ? max : 5), 1),
    RATING_MAX_STARS,
  );
...
export function ratingStarFills(value: number, max: number): number[] {
  ...
  return Array.from({ length: n.max }, (_, i) =>
    n.value >= i + 1 ? 1 : n.value >= i + 0.5 ? 0.5 : 0,
  );
}
...
export function renderRating(options: RenderNodeContentOptions): React.ReactElement {
  ...
  const { value, max } = normalizeRating(
    readNumber(options, 'value', 5),
    readNumber(options, 'max', 5),
  );
  ...
  const label = formatRatingLabel(
    readString(options, 'labelTemplate', '{value} out of {max}'),
    value,
    max,
  );
