...
| 11   | Host integration feedback | Host (funnel builder) dapat menyuplai blocks/templates, menyimpan via `onSave`, merender halaman responsif, dan memakai komponen konversi tanpa workaround. |
...
# Phase 11 — Host Integration Feedback (Funnel Builder)

Sumber: temuan integrasi KUBUILD `0.7.0` sebagai page builder funnel di aplikasi host (jomsini — NestJS + Next.js), 2026-09-24. Epic `EPIC-57` s/d `EPIC-62`, key `STORA-530` s/d `STORA-556`. Epic 57–61 (wave 1) di-merge di branch `feat/host-integration-feedback`; Epic 62 dan STORA-539 (wave 2) belum dikerjakan.
...
Epic key: `EPIC-57` — package target: `@kubuild/core`, `@kubuild/renderer`, `@kubuild/components`.
...
Epic key: `EPIC-58` — package target: `@kubuild/editor`, `@kubuild/react`.
...
Epic key: `EPIC-59` — package target: `@kubuild/renderer`.
...
Epic key: `EPIC-60` — package target: `@kubuild/components`, `@kubuild/renderer`.
...
Epic key: `EPIC-61` — package target: `@kubuild/schema`, `@kubuild/core`, `@kubuild/editor`.
...
Epic key: `EPIC-62` — package target: `docs/`.
